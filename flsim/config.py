...
class AttackCfg:
    ...
    scale: float = 10.0                         # for scaling/signflip (used as magnitude)
    noise_std: float = 0.1                      # for gaussian noise on updates
...
def build_config(args, run_dir: Path):
    ...
    if getattr(args, "partitioner", None) is not None: data.partitioner = args.partitioner
    if getattr(args, "alpha", None) is not None: data.alpha = args.alpha
    if getattr(args, "classes_per_node", None) is not None: data.classes_per_node = args.classes_per_node
    if getattr(args, "shards_per_node", None) is not None: data.shards_per_node = args.shards_per_node
    if getattr(args, "samples_per_client", None) is not None: data.samples_per_client = args.samples_per_client
