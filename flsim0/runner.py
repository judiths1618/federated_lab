        ...
        with open(self.cfg.paths.log_csv, "w", newline="") as f:
            ...
            for r in range(self.cfg.train.rounds):
                ...
                for row in manifest:
                    nid = row["node_id"]
                    writer.writerow([
                        r, f"{acc:.4f}", manifest_cid, nid, row["samples"],
                        f"{row['loss']:.6f}", f"{row['acc']:.6f}",
                        f"{metrics_map.get(nid, {}).get('acc', float('nan')):.4f}",
                        f"{self.contract.features.get(r, {}).get(nid, {}).get('eval_acc', float('nan')):.4f}",
                        f"{(metrics_map.get(nid, {}).get('acc', float('nan')) - self.contract.features.get(r, {}).get(nid, {}).get('eval_acc', float('nan'))):.4f}",
                        row["update_type"], row["model_update_cid"], row["metrics_cid"],
                        f"{self.contract.contribs.get(r, {}).get(nid, float('nan')):.4f}",
                        f"{self.contract.rewards.get(r, {}).get(nid, 0.0):.4f}",
                        f"{self.contract.get_balance(nid):.4f}",
                        f"{self.contract.penalties.get(r, {}).get(nid, 0.0):.4f}",
                        f"{self.contract.nodes.get(nid, {}).get('stake', float('nan')):.4f}",
                        f"{self.contract.nodes.get(nid, {}).get('reputation', float('nan')):.4f}",
                        self.contract.mal_detected.get(r, {}).get(nid, 0),
                        int(nid in set(self.contract.committees.get(r, []))),
                        row["is_malicious"], row["strategy"],
                    ])
